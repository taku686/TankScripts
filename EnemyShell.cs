...
public class EnemyShell : MonoBehaviour
{
    ...
    {
        if (collision.collider.CompareTag("Enemy") || collision.collider.CompareTag("Player") || collision.collider.CompareTag("Shell") || collision.collider.CompareTag("EnemyShell") || collision.collider.CompareTag("DestroyWall") || collision.collider.CompareTag("ExplosionEffect"))
        {
    ...
    {
        if (other.CompareTag("PenetrationShell") || other.CompareTag("DiffuseShell"))
        {
            Instantiate(explosionEffect, transform.position, Quaternion.identity);
            AudioSource.PlayClipAtPoint(explosionSound, transform.position);
            gameObject.SetActive(false);
